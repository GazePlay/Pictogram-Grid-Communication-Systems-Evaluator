...
for i in range(0, len(disTab)):
    ...
    if linkCount < len(linksList):
        # On calcule la disantce entre le pictogramme en haut à gauche
        # avec x=1 et y=1 et celui en bas à doite x=4 et y=8
        squaredDistance2 = (1 - 4) ** 2 + (1 - 8) ** 2
        pictoToPage = math.sqrt(squaredDistance2)

        # Ecriture du résultat dans un fichier
        bowsFile.write("Lien Picto Page" + "\t" + linksList[linkCount][0] + "\t" + linksList[linkCount][1] + "\t" + str(
            pictoToPage * m + n) + "\n")
        linkCount += 1

    # On écrit le lien entre la page et le pictogramme
    # On calcule la disantce entre le lien de la page et des pictogrammes
    # à partir du pictogramme en haut à gauche avec x=1 et y=1
